...
import com.i2c.campaigngenservice.dao.*;
...
public class CommonUtilities
  ...
  public static String calulateDateInfo(String sourceformat, String date,
    ...
    try {
      ...
      System.out.println("Exception in adding Days method--->" + e);
    } //end catch
    return newDate;
  } //end method
...
//				System.out.println(" Final Logger -->" + logger);
  ...
  public static StringBuffer addCharPadding(StringBuffer info, String character,
    ...
    try {
      ...
      System.out.println("  Exception in Adding Character Padding to the String method --->" +
                                      e);
    } //end catch
    return info;
  } //end method
  ...
  public static StringBuffer addCharPadding(String value, String character,
    ...
    System.out.println(" Orginal String for padding is --->" +
                                    info + "<--- String Size is -->" +
                                    info.length());
    System.out.println(" Character string to be padded is --->" +
                                    character);
    System.out.println(" Size to be padded is --->" + size);
    System.out.println(" Field Desc --->" + fieldDesc);
    ...
    try {
      ...
      System.out.println(" Final String After padding is --->" +
                                      info + "<--- String Size is -->" +
                                      info.length());
    }
    catch (Exception e) {
  ...
  public static String calcCheckDigit(String value) {
    ...
    long digitSum = 0;
    try {
      System.out.println(" The Value for Calculating Check Digit is --->" +
                                      value);
      ...
      System.out.println("Exception in Calculating Check Digit Method --->" +
                                      ex);
    } //end catch
    System.out.println("The Final Check Digit is -->" + checkDigit);
    return checkDigit;
  ...
  public static long calculateTimeDifference(String currentDate,
    ...
    try {
      ...
      System.out.println(" Exception in Calculating Time Difference -->" +
                                      ex);
    } //end catch
    System.out.println(
  ...
  public static long calculateCurrentMilliTime(String currentDate,
    ...
    try {
      ...
      System.out.println(" Exception in Calculating Current Time Milli -->" +
                                      ex);
    } //end catch
    System.out.println(
  ...
  public static Vector getMailInfo(String mailID, String instanceName) {
    ...
    try {
      ...
      System.out.println(" CommonUtilities --- getMailInfo ---  Exception in getting mail info from Database--->" +
                                      ex);
    }
    finally {
      if (dbConn != null) {
  ...
  public static Vector getMailInfo(Connection dbConn, String mailID) {
    Vector info = new Vector();
    Statement stmt = null;
    ResultSet rs = null;
    StringBuffer query = new StringBuffer();
  ...
  public static Vector getMailInfo(String mailID, Connection dbConn) {
    ...
    try {
      ...
      System.out.println(" CommonUtilities --- getMailInfo ---  Exception in getting mail info from Database--->" + ex);
    }
    finally {
      try {
        if (rs != null) {
  ...
  public static String emailTokenizer(String value, String delimeter) {
    ...
    try {
      System.out.println( " Convert String Array Method Value is -->" +
                                      value +
                                      "<--- Delimeter -->" + delimeter);
      //Tokenizing the string to create list
      ...
      while (stk.hasMoreTokens()) {
        token = stk.nextToken();
        System.out.println(" Convert String Array Method Value is --- Token--->" +
                                        token);
      ...
      System.out.println(" Exception in convertStringArray method -->" +
                                      ex);
    } //end catch
    System.out.println(" Message Built --- Token--->" +
                                    message.toString());
  ...
  public static Calendar dateString2Calendar(String s, String dateFormat) throws
            Exception {
        SimpleDateFormat df = new SimpleDateFormat(dateFormat);
        Calendar cal = Calendar.getInstance();
        java.util.Date d1 = df.parse(s);
        cal.setTime(d1);
        return cal;
    }
  ...
  public static boolean isReminderValid(String startDate,
                                          String nextDate,
                                          String currentDateFormat,
                                          String newDateFormat,
                                          int reminderFreq,
                                          int reminderIntervel,
                                          int reminderUnit) throws Exception {

        /** Today's date */
    // Calendar now = Calendar.getInstance();
    SimpleDateFormat formatter = new SimpleDateFormat(newDateFormat);

    String startFormatedDate = convertDateFormat(newDateFormat,
                                                 currentDateFormat, startDate);
    String nextFormatedDate = convertDateFormat(newDateFormat,
                                                currentDateFormat, nextDate);

    Calendar now1 = dateString2Calendar(startFormatedDate, newDateFormat);

    System.out.println("Date before Adding-------> " + formatter.format(now1.getTime()));

    long dateDiff = calculateTimeDifference(startFormatedDate,
                                            nextFormatedDate, newDateFormat);

    int reminderDTime = reminderFreq * reminderIntervel;

    if (reminderUnit == 1) {
        now1.add(Calendar.HOUR_OF_DAY, reminderDTime);
    } else if (reminderUnit == 2) {
        now1.add(Calendar.DAY_OF_MONTH, reminderDTime);
    }
    System.out.println("Date after Adding------> " + formatter.format(now1.getTime()));

    long reminderDateDiff = calculateTimeDifference(startFormatedDate,
            formatter.format(now1.getTime()), newDateFormat);

        if (dateDiff > reminderDateDiff) {
            return true;
        } else {
            return false;
        }
