...
/******************************************************************************/
  ...
  public static ArrayList getExtInterfaceConfig(DbConfigBean conn) throws Exception
   ...
   public static Connection getDBConnection(DbConfigBean connObj,
                                            Logger lgr) throws SQLException {
    ...
    lgr.log(I2cLogger.INFO,
            "Got Connection Object Connection String is -->" + connObj.getConnectionString() +
            "<---Connection Name -->" + connObj.getConnectionName() +
            "<-- User -->"+ connObj.getUserID() +
            "<--- Password -->" + connObj.getPasswod() +
            "<--- Connection Pool Size -->" + connObj.getConnectionPool().size());
    ...
    Connection con = DriverManager.getConnection(connObj.getConnectionString(), connObj.getUserID(), connObj.getPasswod());
      ...
      try {
        System.out.println("[DataBaseHandler].[getDBConnection] Calling checkConnection method --- Response received-->" + checkConnection(con, lgr));
      }
      catch (SQLException ex) {
        System.out.println("[DataBaseHandler].[getDBConnection] Exception in the method for checking connection ---> " + ex);
        throw new SQLException(ex.getMessage() );
      }
      return con;
  }//end get connection
