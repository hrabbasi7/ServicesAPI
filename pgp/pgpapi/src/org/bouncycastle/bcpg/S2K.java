    ...
    extends BCPGObject
{
        ...
        throws IOException
    {
        ...
        type = dIn.read();
        CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"S2K --- Constructor --- TYPE --->" + type);
        algorithm = dIn.read();
        CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"S2K --- Constructor --- ALGORITHM --->" + algorithm);
        ...
        if (type != GNU_DUMMY_S2K)
        {
            if (type != 0)
            {
                ...
                CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"S2K --- Constructor --- IV --->" + new String(iv));
            }

            if (type == 3)
            {
                itCount = dIn.read();
                CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"S2K --- Constructor --- IV COUNT--->" + itCount);
            }
        }
        else
        {
            ...
            CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"S2K --- Constructor --- Protection Mode--->" + protectionMode);
        }
    }
