    ...
    implements SymmetricKeyAlgorithmTags
{
        ...
        throws NoSuchProviderException, PGPException
    {
        ...
        CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"PGPEncryptedDataGenerator --- addMethod --- S2K Attributes--- Hash Algorithm---> "  + HashAlgorithmTags.SHA1 + " IV---> " + new String(iv)+ " Iterations---> 0x60");
        methods.add(new PBEMethod(defAlgorithm, s2k, PGPUtil.makeKeyFromPassPhrase(defAlgorithm, s2k, passPhrase, defProvider)));
        CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"PGPEncryptedDataGenerator --- addMethod --- Symmetric Algorithm--->" + defAlgorithm + " defProvider--->" + defProvider);
    }
        ...
        throws IOException, PGPException
    {
        ...
        CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"PGPEncryptedDataGenerator --- addMethod --- Method Size--->"+methods.size());
        if (methods.size() == 0)
        {
        ...
        else if (methods.size() == 1)
        {
            if (methods.get(0) instanceof PBEMethod)
            {
                ...
                CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"PGPEncryptedDataGenerator --- addMethod --- PBEMethod Instance--->"+ m);
                key = m.getKey();
                CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"PGPEncryptedDataGenerator --- addMethod --- Key Instance--->"+ key);
            }
            else
            {
        ...
        CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"PGPEncryptedDataGenerator --- addMethod --- getSymmetricCipherName--->"+ cName);
        if (cName != null)
        {
            try
            {
                ...
                CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"PGPEncryptedDataGenerator --- addMethod --- Cipher Instance--->"+ c);
                c.init(Cipher.ENCRYPT_MODE, key, new IvParameterSpec(new byte[c.getBlockSize()]));
                CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"PGPEncryptedDataGenerator --- addMethod --- Buffer Size--->"+ buffer);
                if (buffer == null)
                {
                    //
                    // we have to add block size + 2 for the generated IV and + 1 + 22 if integrity protected

                    // -------THIS IS IMPORTANT YOU NEED TO SPEND TIME HERE-----
                    if (withIntegrityPacket)
                    {
                ...
                CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"PGPEncryptedDataGenerator --- addMethod --- IV--->"+ new String(inLineIv));
                inLineIv[inLineIv.length - 1] = inLineIv[inLineIv.length - 3];
                inLineIv[inLineIv.length - 2] = inLineIv[inLineIv.length - 4];
                CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"PGPEncryptedDataGenerator --- addMethod --- Integrity Check Byte [9th]--->"+ inLineIv[inLineIv.length - 1]);
                CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"PGPEncryptedDataGenerator --- addMethod --- Integrity Check Byte [10th]--->"+ inLineIv[inLineIv.length - 2]);
                cOut.write(inLineIv);
            ...
            {
              CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_WARNING),"PGPEncryptedDataGenerator --- addMethod --- Exception creating cipher" + e);
              throw new PGPException("Exception creating cipher", e);
            }
        }
        else
        {
            CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_WARNING),"PGPEncryptedDataGenerator --- addMethod --- Cipher is null");
        ...
        throws IOException
    {
        if (cOut != null)
        {
            ...
            if (digestOut != null)
            {
                ...
                try
                {
                    pOut.write(c.doFinal(digestOut.getMessageDigest().digest()));
                    pOut.finish();
                }
                catch (Exception e)
                {
