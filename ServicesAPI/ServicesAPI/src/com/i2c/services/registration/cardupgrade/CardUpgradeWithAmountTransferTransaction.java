...
/**
 * <p>Title:CardUpgradeWithAmountTransferTransaction: A class which is used to upgrade the card </p>
 * <p>Description:This class upgrades the card and transfers the funds from old card into new card such
 * as generating the new card from the older card information and transfer the older card funds into
 * the new card.</p>
...
public class CardUpgradeWithAmountTransferTransaction
    extends Transaction {
  private TransactionRequestInfoObj requestObj = null;
  private Connection dbConn = null;

  /**
   * Constructor of the class which initilizes the database connection and transaction request bean
   * which are used later for processing.
   * @param dbConn Connection
   * @param requestObj TransactionRequestInfoObj
   */

  public CardUpgradeWithAmountTransferTransaction(Connection dbConn,
  ...
  /**
   * This method is used to upgrade the card and transfer funds from older card into the newly
   * generated card. It first validates the following attributs, if found invlalid then it pauses the
   * execution and returns response to client with appropiate response code.<br>
  ...
  public TransactionResponseInfoObj processTransaction() {
    CardUpgradeWithAmountTransferInformationValidator validator = null;
...
//    String cardBalance = null;
    ...
    try {
      CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_CONFIG),
                                      "Method for Processing Card Uprade With Amount Transfer Transaction");

      CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_CONFIG),
                                      "Validating Mandatory Attibutes");
      validator = new CardUpgradeWithAmountTransferInformationValidator(
          requestObj);
      try {
      ...
      if(Double.parseDouble(requestObj.getTransferAmount()) <= 0){
        CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_CONFIG),
                                        "Invalid Transfer Amount received--->" +
                                        requestObj.getTransferAmount());
        clientResposne = prepareClientResponse(requestObj.getExistingCard(),
                                               Constants.CARD_UPGRADE, "13",
                                               "Invalid Transfer Amount received--->" +
                                               requestObj.getTransferAmount(), null);
        clientResposne.setFeeAmount("0.00");
        return clientResposne;
      }
...
//      CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_CONFIG),
//                                      "Checking if amount to transfer not greater than card balance");

//      //Checking if amount to transfer not greater than card balance
//      if (!cuDBHndlr.validateTrasferAmount(requestObj.getExistingCard(),
//                                           requestObj.getTransferAmount())) {
//        //Call Log Transaction
//        //return response
//      }
      ...
      dbDAO.setAmount(requestObj.getTransferAmount());
      ...
      if (!dbResponse.getResponseCode().equalsIgnoreCase(Constants.SUCCESS_CODE)) {
        clientResposne = prepareClientResponse(requestObj.getExistingCard(),
...
//                                          cardBalance + "<---Service Fee--->" +
//                                          serviceFee);
//          if (!verifyCardFunds(cardBalance, serviceFee)) {
//            CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.
//                LOG_CONFIG),
...
//        dbDAO.setServiceId(Constants.CARD_UPGRADE);
 ...
 /**
   ...
   * of the new card it updates the card holder profile and returns the number of the newly generated card.
   * @throws Exception
   * @return String
  */
 ...
 private String performCardRegistrationProcessing() throws Exception{
    ...
    try {
      ...
      CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_CONFIG),
                                      "Exception in performing Card Registration Processing--->" +
                                      ex);
      throw ex;
    }
    return cardNumber;
