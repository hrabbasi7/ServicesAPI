...
public class AdminMailService extends Thread {
  ...
  public AdminMailService(String _message,String _subject) {
    message = _message;
    subject = _subject;
    ServiceMain.lgr.log(LogLevel.getLevel(Constants.LOG_FINEST)," Admin Mail Send Constructor Message -->"+message+"<--- Subject -->"+subject);
  }//end constrcutor
...
//Main Constructor
  public AdminMailService(String _message,String _subject,String toEmailAddresses, String fromAddress) {
    ...
    ServiceMain.lgr.log(LogLevel.getLevel(Constants.LOG_FINEST)," Admin Mail Send Constructor Message -->"+message+"<--- Subject -->"+subject+"<--- toEmails -->"+toEmails+"<--- fromEmail -->"+fromEmail);
  }//end constrcutor


  public void run() {
    ServiceMain.lgr.log(LogLevel.getLevel(Constants.LOG_FINEST)," Admin Mail Service Thread is Started Report -->"+message);
    try {
        for (int i = 0; i < Constants.MAX_EMAIL_RETRY; ++i) {
          ServiceMain.lgr.log(LogLevel.getLevel(Constants.LOG_FINEST),"<--- Maximum  Email Retry -->"+i);
          if (sendAdminReportFailedEmail()) {
            ServiceMain.lgr.log(LogLevel.getLevel(Constants.LOG_FINEST),"<--- Success Email Send Clear Things -->");
            break;
          }//end if
        }//end for
    } catch (Exception ex) {
      ServiceMain.lgr.log(LogLevel.getLevel(Constants.LOG_WARNING)," Exception in Admin Serive Thread -->"+ex);
    }//end catch
  }//end run
  ...
  private boolean sendAdminReportFailedEmail(){
    ...
    try {
      ...
      ServiceMain.lgr.log(LogLevel.getLevel(Constants.LOG_WARNING)," Send Admin Email Method-->"+message+"<-- Subject -->"+subject+"<--- toEmails -->"+toEmails);
      ...
      ServiceMain.lgr.log(LogLevel.getLevel(Constants.LOG_WARNING)," Exception in Send Admin Email -->"+ex);
      flag = false;
    }//end catch
    return flag;
