...
public class PaymentServiceMailer {
  ...
  public PaymentServiceMailer(String emailID, String instanceName, String machineIP,
                       Logger lgr) {
  ...
  public boolean sendEmailNotification() throws Exception {
    ...
    if (mailInfo != null && mailInfo.size() > 0) {
      ...
      lgr.log(LogLevel.getLevel(Constants.LOG_FINEST),
              "PaymentServiceMailer --- Method for sending mail Message --- Mail Info got --- From--->" +
              fromAddress +
      ...
      if (fromAddress != null && fromAddress.trim().length() > 0 && toAddress != null &&
        ...
        lgr.log(LogLevel.getLevel(Constants.LOG_FINEST), "PaymentServiceMailer --- Method for sending mail Message --- Sending Mail Message");
        if (isHTMLText) {
        ...
        lgr.log(LogLevel.getLevel(Constants.LOG_WARNING),
                "PaymentServiceMailer --- Method for sending mail Message --- Mandatory Attributes for sending email are missing --- Cannot Send email");
      ...
      lgr.log(LogLevel.getLevel(Constants.LOG_WARNING),
              "PaymentServiceMailer --- Method for sending mail Message --- Cannot Send email -- No email Info is found in emails table for email id---> " +
              emailID);
...
//      message.append("\n^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n\n");
  ...
  private Vector getMailInfo() {
    ...
    try {
      ...
      lgr.log(LogLevel.getLevel(Constants.LOG_FINEST),
              "PaymentServiceMailer --- Vector size of mail info --->" +
              info.size());
    }
    catch (Exception ex) {
      lgr.log(LogLevel.getLevel(Constants.LOG_WARNING),
              "PaymentServiceMailer --- Excpetion in getting mail info --->" +
              ex);
    }
    return info;
  }
  ...
  private void sendHTMLMessage(String from, String to, String subject,
    ...
    try {
      lgr.log(LogLevel.getLevel(Constants.LOG_FINEST), " PaymentServiceMailer --- sendHTMLMessage ---  Method for sending HTML success notification");
      lgr.log(LogLevel.getLevel(Constants.LOG_FINEST),
      ...
      lgr.log(LogLevel.getLevel(Constants.LOG_WARNING),
              " PaymentServiceMailer --- sendHTMLMessage ---  Exception in sending success email notification--> " +
              ex);
    }
  } //end method
  ...
  private Date getMailSentDate() {
    ...
    try {
      ...
      lgr.log(LogLevel.getLevel(Constants.LOG_WARNING),
              " PaymentServiceMailer --- getMailSentDate --- Exception in getting Sent Mail Date--> " +
              ex);
    }
    return sentDate;
  } //end method
