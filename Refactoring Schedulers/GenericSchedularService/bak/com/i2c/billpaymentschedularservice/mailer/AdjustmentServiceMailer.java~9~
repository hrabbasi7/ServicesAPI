package com.i2c.billpaymentschedularservice.mailer;

import java.util.Vector;
import com.i2c.billpaymentschedularservice.dao.MailMsgDAO;
...
public class AdjustmentServiceMailer {
  ...
  public AdjustmentServiceMailer(String emailID, String instanceName, String machineIP, Logger lgr) {
  ...
  public AdjustmentServiceMailer(String emailID, String instanceName, String machineIP,
  ...
  public boolean sendEmailNotification() throws Exception {
    ...
    lgr.log(LogLevel.getLevel(Constants.LOG_FINEST),
            "AdjustmentServiceMailer --- Method for sending mail Message --->");

    mailInfo = getMailInfo();
    lgr.log(LogLevel.getLevel(Constants.LOG_FINEST),
            "AdjustmentServiceMailer --- Method for sending mail Message --- Mail Info size got --->" +
            mailInfo.size());
    if (mailInfo != null && mailInfo.size() > 0) {
      ...
      lgr.log(
          ...
          "AdjustmentServiceMailer --- Method for sending mail Message --- Mail Info got --- From--->" + fromAddress +
      ...
      lgr.log(
          LogLevel.getLevel(Constants.LOG_FINEST),
          "AdjustmentServiceMailer --- Method for sending mail Message --- Final Msg Built");

      if (fromAddress != null && fromAddress.trim().length() > 0 && toAddress != null &&
          toAddress.trim().length() > 0 && mailMsg != null &&
          mailMsg.trim().length() > 0) {
        lgr.log(
            LogLevel.getLevel(Constants.LOG_FINEST), "AdjustmentServiceMailer --- Method for sending mail Message --- Sending Mail Message");
        if(isHTMLText){
        ...
        lgr.log(
            LogLevel.getLevel(Constants.LOG_WARNING), "AdjustmentServiceMailer --- Method for sending mail Message --- Mandatory Attributes for sending email are missing --- Cannot Send email");
        throw new Exception("Adjustment Service --- Mandatory Attributes for sending email are missing --- Cannot Send email--->" +
                            emailID);
      }
    }
    else {
      lgr.log(
          ...
          "AdjustmentServiceMailer --- Method for sending mail Message --- Cannot Send email -- No email Info is found in emails table for email id---> " + emailID);
      throw new Exception("ACH Load Batch --- Cannot Send email -- No email Info is found in emails table for email id---> " +
                          emailID);
    }
    return isSent;
  }
  ...
  private String prepareMessage(String msgHdr, String msgBody, String msgFtr) {
    ...
    if (msgHdr != null && msgHdr.trim().length() > 0) {
      ...
      lgr.log(
          LogLevel.getLevel(Constants.LOG_FINEST),
          "AdjustmentServiceMailer --- Mail Msg build so far --->" + message);
    }
...
//      message.append("\n^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n\n");
  ...
  private Vector getMailInfo() {
    ...
    try {
      lgr.log(
          LogLevel.getLevel(Constants.LOG_FINEST),
          "AdjustmentServiceMailer --- Method for getting mail info for email id --->" +
          emailID);
      mailHnd = new MailMsgDAO(emailID);
      info = mailHnd.getMailInfo(dbConn);
      lgr.log(
          LogLevel.getLevel(Constants.LOG_FINEST),
          "AdjustmentServiceMailer --- Vector size of mail info --->" + info.size());
    }
    catch (Exception ex) {
      lgr.log(
          LogLevel.getLevel(Constants.LOG_WARNING),
          "AdjustmentServiceMailer --- Excpetion in getting mail info --->" + ex);
    }
    return info;
  }
  ...
  private void sendHTMLMessage(String from, String to, String subject,
    ...
    try {
      lgr.log(
          LogLevel.getLevel(Constants.LOG_FINEST), " AdjustmentServiceMailer --- sendHTMLMessage ---  Method for sending HTML success notification");
      lgr.log(
          LogLevel.getLevel(Constants.LOG_FINEST),
          " AdjustmentServiceMailer --- sendHTMLMessage ---  Info received --- From--->" +
      ...
      lgr.log(
          LogLevel.getLevel(Constants.LOG_WARNING),
          " AdjustmentServiceMailer --- sendHTMLMessage ---  Exception in sending success email notification--> " + ex);
    }
  } //end method
  ...
  private Date getMailSentDate() {
    ...
    try {
      ...
      if (date != null && date.trim().length() > 0) {
        ...
        lgr.log(
            LogLevel.getLevel(Constants.LOG_FINEST),
            " AdjustmentServiceMailer --- getMailSentDate --- Got Sent Mail Date--> " +
            sentDate);
      }
    }
    catch (Exception ex) {
      lgr.log(
          LogLevel.getLevel(Constants.LOG_WARNING),
          " AdjustmentServiceMailer --- getMailSentDate --- Exception in getting Sent Mail Date--> " + ex);
    }
    return sentDate;
  } //end method
} //end class
