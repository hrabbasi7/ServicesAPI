package com.i2c.billpaymentschedularservice.monitor;
...
public class PaymentServiceMonitor
    extends Thread {
  ...
  public PaymentServiceMonitor(String instanceName,
                               String serviceName,
                               Logger lgr) {
  ...
  public void run() {
    ...
    lgr.log(LogLevel.getLevel(Constants.LOG_FINEST),
            " Payment Service Monitor Service Thread is Started --- Instance Name-->" +
    ...
    while (true) {
      try {
        ...
        if (alertMessage != null) {
          ...
          lgr.log(LogLevel.getLevel(Constants.LOG_FINEST),
                  " Payment Service Monitor Service Thread detected problem in the execution of Payment Service Thread for instance --->" +
                  instanceName +
          ...
          lgr.log(LogLevel.getLevel(Constants.LOG_FINEST),
                  " Payment Service Monitor Service Thread detected no problem in the execution of Payment Service  Service Thread for instance --->" +
                  instanceName +
        ...
        lgr.log(LogLevel.getLevel(Constants.LOG_WARNING),
                " Exception in Payment Service Monitor Service Thread --->" +
                ex);
      }
      catch (Throwable th) {
        System.out.println(
            "Severe Exception in Payment Service Monitor Service Thread --->" +
            th.getStackTrace());
      }
    } //end while
  }
