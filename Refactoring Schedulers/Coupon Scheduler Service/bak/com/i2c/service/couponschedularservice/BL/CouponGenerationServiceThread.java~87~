...
public class CouponGenerationServiceThread extends BaseHandler {
  ...
  public CouponGenerationServiceThread(String instanceName,
                                       SchedulerHangNotifier couponMonitor) throws Exception{
    ...
    this.couponLogger.setLevel(CommonUtilities.getLogLevel(Constants.LOG_DEBUG_LEVEL));
...
//  public void run() {
      ...
      while (true) {
        try {
          ...
          if (serviceHome.processScheduler(Constants.USE_COUPON_GEN, dbConn)
            ...
            couponLogger.log(I2cLogger.FINE,"[CouponGenerationServiceThread].[run-mainBusinessLogic]<---invoking coupon generation service for this instance--->");
            invokeCouponGeneration(serviceHome);
            couponLogger.log(I2cLogger.FINE,"[CouponGenerationServiceThread].[run-mainBusinessLogic]<---coupon generation service invoked successfully, sleeping for --->" + Constants.COUPON_GEN_SLEEP_TIME + " millisecs");

            success = true;
          }

          if (serviceHome.processScheduler(Constants.USE_GEN_CHARGE_TRANS, dbConn)
              && CommonUtilities.checkPrimaryScheduler()) {
            couponLogger.log(I2cLogger.FINE,"[CouponGenerationServiceThread].[run-mainBusinessLogic]<---invoking <Generate Charge Trans> service for this instance--->");
            invokeGenerateChargeTrans(serviceHome);
            couponLogger.log(I2cLogger.FINE,"[CouponGenerationServiceThread].[run-mainBusinessLogic]<---<Generate Charge Trans> invoked successfully, sleeping for --->" + Constants.COUPON_GEN_SLEEP_TIME + " millisecs");
          ...
          if (success == true){
            ...
            couponLogger.log(I2cLogger.FINE,
                             "[CouponGenerationServiceThread].[run-mainBusinessLogic] " +
                             "Coupon generation OR Generation Charge Trans service is not allowed for this instance-->" + instanceName);
  ...
  private void invokeGenerateChargeTrans(CouponGenerationServiceDAO serviceHome) {
    try {
      couponLogger.log(I2cLogger.INFO,
                       "[CouponGenerationServiceThread].[invokeGenerateChargeTrans] Method for invoking Generate Charge Trans DB API ");

      serviceHome.callGenerateChargeTrans();

      couponLogger.log(I2cLogger.INFO,
                       "[CouponGenerationServiceThread].[invokeGenerateChargeTrans] Method for invoking Generate Charge Trans DB API completed successfully");

    }catch (Exception ex) {
      couponLogger.log(I2cLogger.WARNING,
                       "[CouponGenerationServiceThread].[invokeGenerateChargeTrans] Exception in method for invoking Generate Charge Trans DB API--->" + ex);
      ex.printStackTrace();
    }
  }
