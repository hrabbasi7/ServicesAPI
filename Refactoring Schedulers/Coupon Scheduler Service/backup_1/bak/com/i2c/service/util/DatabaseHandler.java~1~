...
public static Connection getFCConnection(String className,String instanceName) throws SQLException {
  DbConnectionInfoObj connObj = null;
  CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"Getting FC DB connection Instance Name -->"+instanceName+" Connection Instance Size -->"+LoadProperties.instanceConnectionsTable.size());
  try {
    Class.forName(Constants.DB_DRIVER_NAME);
  } catch (Exception e){}
  ...
  CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST)," Got FC Connection Object Connection String is -->"+connObj.getFcConnectionString()+"<---Connection Name -->"+connObj.getConnectionName()+"<-- User -->"+connObj.getFcUserID()+"<--- Password -->"+connObj.getFcPasswod());
  //Creating Instance connection
