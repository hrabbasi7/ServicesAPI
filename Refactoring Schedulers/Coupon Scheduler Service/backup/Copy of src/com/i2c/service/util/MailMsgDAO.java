...
public class MailMsgDAO {
  ...
  public Vector getMailInfo(String instanceName) throws Exception{
    ...
    try {
      CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"Method for getting Email info for ID-->" + emailID + "<--InstanceName-->" + instanceName);
      query.append("select * from emails where email_id = ");
      CommonUtilities.buildQueryInfo(query,emailID,true);
      CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"Query for getting Email info--->" + query);
      ...
      if(rs.next()){
        ...
        CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"No Email info found for given email ID --->" + emailID);
        throw new Exception("Unable to find email information from 'emails' table for provided emailID--->" + emailID);
      }
    }
    catch (Exception ex) {
      CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_WARNING),"Exception in getting mail info --->" + ex);
  ...
  public Vector getMailInfo(Connection dbConn) throws Exception{
    ...
    try {
      CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"Method for getting Email info for ID-->" + emailID);
      query.append("select * from emails where email_id = ");
      CommonUtilities.buildQueryInfo(query,emailID,true);
      CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),"Query for getting Email info--->" + query);
