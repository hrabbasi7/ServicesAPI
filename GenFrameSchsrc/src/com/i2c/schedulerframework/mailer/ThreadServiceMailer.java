...
public class ThreadServiceMailer {
  ...
  public ThreadServiceMailer(String emailID, String instanceName, String machineIP,
                     Logger lgr) {
  ...
  public boolean sendEmailNotification() throws Exception {
    ...
    lgr.log(I2cLogger.FINEST,
            "ThreadServiceMailer --- Method for sending mail Message --->");

    mailInfo = getMailInfo();

    lgr.log(I2cLogger.FINEST,
            "ThreadServiceMailer --- Method for sending mail Message --- Mail Info size got --->" +
            mailInfo.size());
    if (mailInfo != null && mailInfo.size() > 0) {
      ...
      lgr.log(I2cLogger.FINEST,
              "ThreadServiceMailer --- Method for sending mail Message --- Mail Info got --- From--->" +
              fromAddress +
      ...
      lgr.log(I2cLogger.FINEST,
              "ThreadServiceMailer --- Method for sending mail Message --- Final Msg Built");
      ...
      if (fromAddress != null && fromAddress.trim().length() > 0 && toAddress != null &&
        ...
        lgr.log(I2cLogger.FINEST,
                "ThreadServiceMailer --- Method for sending mail Message --- Sending Mail Message");
        ...
        lgr.log(I2cLogger.WARNING,
                "ThreadServiceMailer --- Method for sending mail Message --- Mandatory Attributes for sending email are missing --- Cannot Send email");
      ...
      lgr.log(I2cLogger.WARNING,
              "ThreadServiceMailer --- Method for sending mail Message --- Cannot Send email -- No email Info is found in emails table for email id---> " +
              emailID);
  ...
  private String prepareMessage(String msgHdr, String msgBody, String msgFtr) {
    ...
    if (msgHdr != null && msgHdr.trim().length() > 0) {
      ...
      lgr.log(I2cLogger.FINEST,
              "ThreadServiceMailer --- Mail Msg build so far --->" + message);
    }
  ...
  private Vector getMailInfo() {
    ...
    try {
      ...
      lgr.log(I2cLogger.FINEST,
              "ThreadServiceMailer --- Method for getting mail info for email id --->" +
              emailID);
      mailHnd = new MailMsgDAO(emailID);
      info = mailHnd.getMailInfo(dbConn);
      lgr.log(I2cLogger.FINEST,
              "ThreadServiceMailer --- Vector size of mail info --->" +
              info.size());
    }
    catch (Exception ex) {
      lgr.log(I2cLogger.WARNING,
              "ThreadServiceMailer --- Excpetion in getting mail info --->" + ex);
    }
    return info;
  }
  ...
  private void sendHTMLMessage(String from, String to, String subject,
    ...
    try {
      lgr.log(I2cLogger.FINEST,
              " ThreadServiceMailer --- sendHTMLMessage ---  Method for sending HTML success notification");
      lgr.log(I2cLogger.FINEST,
              " ThreadServiceMailer --- sendHTMLMessage ---  Info received --- From--->" +
      ...
      lgr.log(I2cLogger.WARNING,
              " ThreadServiceMailer --- sendHTMLMessage ---  Exception in sending success email notification--> " +
              ex);
    }
  } //end method
  ...
  private Date getMailSentDate() {
    ...
    String date = null;
    try {
      date = CommonUtilities.getCurrentFormatDateTime(BaseConstants.ACH_DATE_TIME_FORMAT);
      if (date != null && date.trim().length() > 0) {
        sentDate = sdf.parse(date);
        lgr.log(I2cLogger.FINEST,
                " ThreadServiceMailer --- getMailSentDate --- Got Sent Mail Date--> " +
                sentDate);
      }
    }
    catch (Exception ex) {
      lgr.log(I2cLogger.WARNING,
              " ThreadServiceMailer --- getMailSentDate --- Exception in getting Sent Mail Date--> " +
              ex);
    }
    return sentDate;
  } //end method
