...
import com.i2c.filestore.retriever.FileRetriever;
import com.i2c.filestore.retriever.factory.FileRetrieverFactory;
import com.i2c.filestore.persister.PersistenceType;
import com.i2c.filestore.info.RetrieveCriteria;
import com.i2c.filestore.info.impl.RetrieveCriteriaImpl;
import com.i2c.filestore.info.FileInfo;
import com.i2c.filestore.persister.factory.FilePersisterFactory;
import com.i2c.filestore.persister.FilePersister;
import com.i2c.filestore.info.FileTypeInfo;
import com.i2c.filestore.info.impl.FileInfoImpl;
import com.i2c.filestore.info.impl.FileTypeInfoImpl;
...
//				System.out.println(" Final Logger -->" + logger);
  ...
  public static MailInfoObj getMailInfo(Connection dbConn, String mailID) {
    MailInfoObj mailObj = null;
  ...
  public static boolean checkPrimaryScheduler(){
      if (Constants.SCHEDULER_TYPE.equalsIgnoreCase("S")){
        try {
          Socket skt = new Socket(Constants.SCHEDULER_IP, Constants.SCHEDULER_PORT);
          if (skt.isConnected()) {
            CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_FINEST),
            ...
            CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.
                LOG_WARNING),
                                            "--->Primary Scheduler Socket NOT Connected at " +
                                            Constants.SCHEDULER_IP + ":" +
                                            Constants.PORT_NUMBER);
            return true;
          }
          ...
          CommonUtilities.getLogger().log(LogLevel.getLevel(Constants.LOG_WARNING),
                                          "UnknownHostException: PRIMARY SCHEDULER NOT WORKING on " +
                                          Constants.SCHEDULER_IP + ":" +
                                          Constants.PORT_NUMBER);
          e.printStackTrace();
          return true;
  ...
  /**
   * Get file from the DB on the bases of name provided on the local path given
   * @param con Connection
   * @param fileNameToGet String
   * @param localPathToWriteFile String
   * @return ArrayList -- List of files which fulfil the criteria
   * @throws Exception
   */
  public static ArrayList getFileFromDB(Connection con, String fileNameToGet, String localPathToWriteFile) throws Exception {
      try {
          FileRetriever retriver = FileRetrieverFactory.getInstance().
                                   getFileRetriever(PersistenceType.TEXT);
          RetrieveCriteria criteria = new RetrieveCriteriaImpl();
          criteria.setFileName(fileNameToGet);
          criteria.setLocationPath(localPathToWriteFile);
          ArrayList<FileInfo> info = null;
          info = retriver.retrieveFiles(con, criteria);
          return info;
      }catch(Exception ex) {
          throw new Exception("Exception while getting File in CommonUtils.getFileFromDB()--->" +
   ...
   * @param con Connection
   * @param pathToReadFrom String
   * @param fileName String
   * @param fileType int
   * @return int
   * @throws Exception
   */
  public static int saveFileInBD(Connection con, String pathToReadFrom, String fileName, int fileType) throws Exception {
      int fileId = -1;
      FilePersisterFactory persistFactory = null;
      FilePersister persister = null;
      FileInfo fileInfo = null;
      FileTypeInfo typeInfo = null;
      String file = null;

      try {
          persistFactory = FilePersisterFactory.getInstance();
          persister = persistFactory.getFilePersister(PersistenceType.TEXT);

          fileInfo = new FileInfoImpl();
          typeInfo = new FileTypeInfoImpl();
          typeInfo.setId(fileType);
          fileInfo.setFileType(typeInfo);
          fileInfo.setFileName(fileName);
          file = pathToReadFrom + File.separator + fileName;
          fileInfo.setFilePath(file);
          fileInfo.setLogDate(CommonUtilities.getCurrentFormatDateTime(Constants.DATE_FORMAT));
          fileInfo.setLogTime(CommonUtilities.getCurrentFormatDateTime(Constants.TIME_FORMAT));
          fileId = persister.persistFile(con, fileInfo);
      } catch (Exception ex) {
          throw new Exception("Exception while save File in CommonUtils.saveFileInBD()--->" +
                              pathToReadFrom + fileName + "\n\nStack Trace----->" +
                              getStackTrace(ex));
      }
      return fileId;
  }
