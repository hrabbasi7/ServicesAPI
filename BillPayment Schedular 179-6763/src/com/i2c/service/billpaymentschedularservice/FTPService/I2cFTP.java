...
//      }
   ...
   * @throws IOException
   */
  public boolean listFiles(String ftpFileNameToFind) throws
      IOException, FTPException {
    boolean isExist = false;
    log.debug("Listing Files at FTP site");
    // check if connected
    if (ftp == null) {
      return isExist;
    }
    ...
    log.debug("Connected with FTP site for listing of files");
    try {
//      log.info("<---->Downloading<-----> File from : " + ftp.pwd() + " to " + localPath);
      String[] listFile = ftp.dir(ftpFileNameToFind);
