...
import com.i2c.payeeinfoservice.*;
...
//  }
    ...
    public long insertValues(String insertquery, Connection dbConn) throws
        ...
        String serialQuery = "";
        boolean flag = true;
        PayeeMapper.lgr.log(I2cLogger.FINE, " Query for inseting values -->\n" +
                              insertquery);
//    lgr.log(I2cLogger.FINE, " Query for inseting values -->\n" + insertquery);
        try {
            ...
            if (insertquery.indexOf(Constants.INSERT_QUERY_INTO_VALUE) > -1) {
                ...
                PayeeMapper.lgr.log(I2cLogger.FINE,
                                      " Serail Number Query -->\n" +
                                      serialQuery);
    ...
    public boolean checkValueExist(Connection dbConn, String queryString) throws
        ...
        PayeeMapper.lgr.log(I2cLogger.FINE, "<-- Query for Checking Value Exist -->\n" +
                         queryString);
        ...
        try {
            ...
            PayeeMapper.lgr.log(I2cLogger.FINE, "SQLException in Checking Value Exist ----> " + e);
            throw new CheckValueExistExcep(e.getErrorCode(), e);
        } catch (Exception e) {
            PayeeMapper.lgr.log(I2cLogger.FINE, "Exception in Checking Value Exist ----> " + e);
        ...
        try {
            ...
            if (retVal.size() > 0) {
                ...
                if (strVal.equalsIgnoreCase(Constants.IS_TRUE)) {
                  ...
                  PayeeMapper.lgr.log(I2cLogger.FINE,  " Use_Scheduler_Service Value --> NULL");
                    return useServcieFlag;
                }
            } else {
                PayeeMapper.lgr.log(I2cLogger.FINE,
    ...
    public boolean processScheduler(String serviceName, String instanceName) throws
        ...
        try {
            ...
            if (retVal.size() > 0) {
                String strVal = retVal.elementAt(0).toString();
                if (strVal.equalsIgnoreCase(Constants.IS_TRUE)) {
                  PayeeMapper.lgr.log(I2cLogger.FINE, " Value Found for specified service -->" + Constants.IS_TRUE);
                    return useServcieFlag;
                } else if (strVal.equalsIgnoreCase(Constants.IS_FALSE)) {
                  PayeeMapper.lgr.log(I2cLogger.FINE, " Use_Scheduler_Service Value --> " + Constants.IS_FALSE);
                    useServcieFlag = false;
                } else {
                  PayeeMapper.lgr.log(I2cLogger.FINE, " Use_Scheduler_Service Value --> NULL");
                    return useServcieFlag;
        ...
        try {
            ...
            for (int i = 0; i < counter; i++) {
                ...
                try {
                  ...
                  PayeeMapper.lgr.log(I2cLogger.FINE,  " BaseHome --- verfiyConnection --- Creating Statment Object---");
                    stmt = dbConn.createStatement();
                    PayeeMapper.lgr.log(I2cLogger.FINE, " BaseHome --- verfiyConnection --- Executing Query---");
                    rs = stmt.executeQuery(query);
                    PayeeMapper.lgr.log(I2cLogger.FINE, " BaseHome --- Verfiy Connection --- Connection is valid -- Closing ResultSet");
                    //Closing the valid statment
                    rs.close();
                    stmt.close();
                    PayeeMapper.lgr.log(I2cLogger.FINE, " BaseHome --- Verfiy Connection --- Connection is valid -- Breaking Loop");
                    break;

                } catch (Exception ex) {
                    PayeeMapper.lgr.log(I2cLogger.FINE, " BaseHome --Cuurent Count -->" + i + "<--- Counter -1 -->" +
                                          (counter - 1) +
                    ...
                    PayeeMapper.lgr.log(I2cLogger.FINE, " BaseHome --- Creating new Connection --- Argumnets passed to DatabaseHandler.getConnection --- ClassName---> BaseHome -- InstanceName--->" + instanceName);

                    //Close exisiting dump connection
                    try {
                        PayeeMapper.lgr.log(I2cLogger.FINE, " Closing existing dump connetion --->");
                        if (dbConn != null)
                            DatabaseHandler.returnConnection(dbConn, "BaseHome");
                        dbConn = null;
                    } catch (Exception e) {
                        PayeeMapper.lgr.log(I2cLogger.FINE, " Exception in closing existing dump connection -->" + e);
                    } //end catch
                    ...
                    try {
                        ...
                        PayeeMapper.lgr.log(I2cLogger.FINE, " BaseHome --- verfiyConnection --- Exception in Creating New Connection --->" +
                                              ex1);
                    } //end catch
                    PayeeMapper.lgr.log(I2cLogger.FINE, " BaseHome --- verfiyConnection --- Got New DBConn--->" + dbConn);
                } //end catch
