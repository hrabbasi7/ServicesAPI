...
public class MailMsgDAO {
  ...
  public Vector getMailInfo(String instanceName) throws Exception{
    ...
    try {
      PayeeInfoMain.lgr.log(I2cLogger.FINEST,"Method for getting Email info for ID-->" + emailID + "<--InstanceName-->" + instanceName);
      query.append("select * from emails where email_id = ");
      CommonUtilities.buildQueryInfo(query,emailID,true);
      PayeeInfoMain.lgr.log(I2cLogger.FINEST,"Query for getting Email info--->" + query);
      ...
      if(rs.next()){
        ...
        PayeeInfoMain.lgr.log(I2cLogger.FINEST,"No Email info found for given email ID --->" + emailID);
        throw new Exception("Unable to find email information from 'emails' table for provided emailID--->" + emailID);
      }
    }
    catch (Exception ex) {
      PayeeInfoMain.lgr.log(I2cLogger.WARNING,"Exception in getting mail info --->" + ex);
  ...
  public Vector getMailInfo(Connection dbConn) throws Exception{
    ...
    try {
      PayeeInfoMain.lgr.log(I2cLogger.FINEST,"Method for getting Email info for ID-->" + emailID);
      query.append("select * from emails where email_id = ");
      CommonUtilities.buildQueryInfo(query,emailID,true);
      PayeeInfoMain.lgr.log(I2cLogger.FINEST,"Query for getting Email info--->" + query);
